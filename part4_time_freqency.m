...
% for subj = 4:24%length(subjects) %186 - BA5 didnt work, 346- BY2
% %     try
...
for subj = 4:length(subjects) %186 - BA5 didnt work, 346- BY2
    %     try
    datapath = strcat('\\130.60.169.45\methlab\Neurometric\Antisaccades\new_paper\martyna\',subjects{subj});
    cd (datapath)
    ...
    tfrprosaccleft = ft_freqdescriptives(cfg, tfrprosacc); 
    save tfrprosaccleft tfrprosaccleft
    
    cfg                 = [];
    cfg.trials = find(tfrprosacc.trialinfo(:,2) == 1 & tfrprosacc.trialinfo(:,1) == 1);%0 jest left
    tfrprosaccright = ft_freqdescriptives(cfg, tfrprosacc); 
    save tfrprosaccright tfrprosaccright
...
for subj = 4:length(subjects) %186 - BA5 didnt work, 346- BY2
    ...
    tfrantisaccleft = ft_freqdescriptives(cfg, tfrantisacc); 
    save tfrantisaccleft tfrantisaccleft
    
    cfg                 = [];
    cfg.trials = find(tfrantisacc.trialinfo(:,2) == 1 & tfrantisacc.trialinfo(:,1) == 1);%0 jest left
    tfrantisaccright = ft_freqdescriptives(cfg, tfrantisacc); 
    save tfrantisaccright tfrantisaccright
    %% HERE BASELINING STARTS!!
    %% do baseline first for sacc locked data
    cfg                 = [];
    cfg.baseline        = [-.55 -.25];
